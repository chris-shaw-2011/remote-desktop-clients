...
74748b66ef0c96225070f20673b2c497e340ccf7b327d0800d947be24d652687
```

## Windows-key capture persistence after updates

The Windows-key accessibility service has a separate lifecycle problem from
RDP keyboard translation. Horizon may remove the service component from
`enabled_accessibility_services` when aRDP is replaced, so a correctly scoped
and implemented service is no longer bound after an update. Reconnecting RDP
cannot repair that Android setting.

aRDP must not try to enable its own accessibility service. Android deliberately
reserves writes to secure settings for privileged callers, and changing the
`ACCESS_RESTRICTED_SETTINGS` AppOp requires platform or installer authority.
The existing Quest Wireless ADB helper already receives a one-time
`WRITE_SECURE_SETTINGS` grant and is therefore the narrow place to restore the
service setting. The AppOp itself is marked non-resettable by Android; the host
connection script can still reassert it through its legitimate ADB shell
authority to tolerate vendor behavior.

### Persistence plan

- [x] Add an idempotent helper operation that preserves every other enabled
  accessibility service, adds aRDP's `SystemKeyCaptureService`, and enables the
  accessibility framework.
- [x] Run that operation when the helper launches, after boot, and in its
  persisted job; have aRDP's update receiver request it after package replacement.
- [x] Have aRDP explicitly request the helper operation when its process starts,
  covering an update followed by the first aRDP launch without requiring the
  helper UI to be opened.
- [x] Make the wireless ADB connection script reassert the restricted-settings
  AppOp and invoke the same helper operation, while preserving its existing
  single-line endpoint output.
- [x] Document the one-time grant and the remaining security boundary, add
  focused list-preservation tests, build both APKs, and rerun the complete
  keyboard validation.
- [ ] Install both updated APKs on Quest and verify that Windows-key capture
  remains active after replacing aRDP and after rebooting the headset.

### Persistence validation

The helper regressions and both debug APK builds pass:

```text
./gradlew :questAdbHelper-app:testDebugUnitTest
  3 tests, 0 failures
./gradlew :questAdbHelper-app:assembleDebug :aRDP-app:assembleDebug
bash -n questAdbHelper-app/connect.sh
git diff --check
```

The packaged manifests contain aRDP's `MY_PACKAGE_REPLACED` receiver and the
helper's explicit restoration receiver. The artifact SHA-256 values are:

```text
74748b66ef0c96225070f20673b2c497e340ccf7b327d0800d947be24d652687  aRDP-app/build/outputs/apk/debug/aRDP-app-arm64-v8a-debug.apk
2a6b56bd52021bc9eb3c15d464851333b57ea63897962ac2a5e736d4103477e1  questAdbHelper-app/build/outputs/apk/debug/questAdbHelper-app-debug.apk
```
