...
Status: implemented and locally validated on August 21, 2026. Physical Quest 3
...
The override is in the aRDP application manifest rather than the shared bVNC
manifest. This keeps the connection grid in its existing launcher task, lets it
be reopened to launch more panels, and avoids changing the other remote desktop
...
| CI validation | `.github/workflows/build-ardp.yml` | Runs registry tests and checks the packaged activity flags before signing. |
...
- [x] Preserve the connection grid as the launcher task.
...
- [x] Preserve the existing pause, stop, explicit-disconnect, and destruction
  lifecycle behavior.
- [x] Add unit tests, packaged-manifest checks, CI ordering, and personal-build
  documentation.
...
The CI workflow runs those unit tests before the release build, verifies the
application ID, version metadata, ARM64-only native libraries, and packaged
activity flags, then zip-aligns, signs, and verifies the APK using repository
secrets. Local validation stopped at the unsigned APK because the release
keystore is intentionally available only to CI.
