...
## Device-test corrections

- September 14, 2026: defer graphics-settings callbacks, drawable allocation,
  and framebuffer replay for an attached monitor until `RemoteCanvasHandler`
  has installed that canvas's connection, handler, pointer, and mode parameters.
  A Quest 3 device trace showed the second panel receiving
  `GRAPHICS_SETTINGS_RECEIVED` while its monitor-local connection was still
  null, causing a null monitor-enter at `RemoteCanvasHandler.handleMessage`.
  Late panels now receive the current graphics-settings event and framebuffer
  once they are ready; the handler also treats an unexpectedly early settings
  event defensively rather than crashing.
